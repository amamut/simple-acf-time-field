...
class Simple_ACF_Time_Field extends acf_Field
{
   ...
   */
	function create_field($field)	{
		// vars
    if ( isset($field['24_hour']) ) {
      $hour = floor($field['value']/60);
      $minute = $field['value']%60;
    } else {
      $hour = date('g', strtotime($field['value']));
      $minute = date('i', strtotime($field['value']));
      $meridiem = date('A', strtotime($field['value']));
      $field['24_hour'] = false;
    }
    ...
    <select name="<?php echo $field['name']; ?>[minute]">
      <?php
      for($i=0; $i<=59; ++$i) {
        $minutes = date('i', strtotime('12:' . $i));
        echo '<option value="' . $minutes . '" ' . ($minute==$minutes ? 'selected="selected"' : '') . '>' . $minutes . '</option>';
      }?>
    </select>
    <?php if(!$field['24_hour']): ?>
    <select name="<?php echo $field['name']; ?>[meridiem]">
      <option value="am" <?php echo $meridiem == 'AM' ? 'selected="selected"' : ''; ?>>AM</option>
      <option value="pm" <?php echo $meridiem == 'PM' ? 'selected="selected"' : ''; ?>>PM</option>
    </select>
    <?php endif;
	}
   ...
   */
    ...
    if($field['24_hour']) {
      ...
      $val = $value['hour'] . ':' . $value['minute'] . ' ' . $value['meridiem'];
    }
